...
Extrinsic (in contrast to intrinsic) requires an additional dataset. The extrinsic topic coherence measure takes the top N words in each topic and sees how often does the word pair appeasr in a common corpus (e.g., Wikipedia). 
...
For simplicity and performance reasons (loading Wikipeida is a very slow), we use the Yelp dataset and split it into training (1k reviews) and a common corpus (19K reviews), testing the model on the latter part.  But notice this is not a common approach, and, in practice, should be performed on a larger corpus.
...
NPMI = function(DT, m,l){  
  ...
  p_ml = length(which(DT[,m] >0 & DT[,l] >0))  / (number_of_documents * number_of_documents)
...
compute_c <- function(LDA_model, dataset,  top_N, method=c("LCP", "NPMI"), top_K= 0){
  ...
  if(method == "LCP")
    ...
    method = NPMI
  top_words <- apply(t(LDA_model), 2, FUN = function(x) sort(x, decreasing = T,index.return = T)$ix[1:top_N])
...
```{r}
small_yelp <- yelp_matrix[1:10000,]
LDA_models <- c()
for(k in seq(10,100,10)){
  model <- LDA(small_yelp, control=list(iter=10, seed=SEED), k=k, method="Gibbs")
  LDA_models <- c(LDA_models, model)
}
```
